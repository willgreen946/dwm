 ...
 * on each monitor. Each client contains a bit array to indicate the tags of
 * a client.
 *
...
struct Pertag
{
  ...
  int nmasters[LENGTH (tags) + 1];            /* number of windows in
                                               * master area */
  float mfacts[LENGTH (tags) + 1];            /* mfacts per tag */
  unsigned int sellts[LENGTH (tags) + 1];     /* selected layouts */
  const Layout *ltidxs[LENGTH (tags) + 1][2]; /* matrix of tags and
                                               * layouts indexes  */
  int showbars[LENGTH (tags) + 1];            /* display bar for the
                                               * current tag */

  int drawwithgaps[LENGTH (tags) + 1]; /* gaps toggle for each
                                        * tag */
...
manage (Window w, XWindowAttributes *wa)
{
  ...
  configure (c); /* propagates border_width, if size doesn't
                  * change */
  updatewindowtype (c);
  updatesizehints (c);
  updatewmhints (c);
...
resizeclient (Client *c, int x, int y, int w, int h)
{
  ...
  if (!selmon->pertag->drawwithgaps[selmon->pertag->curtag]
      ...
      (((nexttiled (c->mon->clients) == c
         && !nexttiled (c->next)) /* these two first lines
                                   * are the only ones
                                   * changed. if you are
                                   * manually patching and
                                   * have noborder
                                   * installed already,
                                   * just change these
                                   * lines; or conversely,
...
sendmon (Client *c, Monitor *m)
{
  ...
  detachstack (c);
  c->mon = m;
  c->tags = m->tagset[m->seltags]; /* assign tags of target
                                    * monitor */
  attach (c);
  attachstack (c);
  focus (NULL);
  ...
  for (m = mons; m; m = m->next)
    {
      ...
      m->barwin = XCreateWindow (
          dpy, root, m->wx, m->by, m->ww, bh, 0, depth, InputOutput, visual,
